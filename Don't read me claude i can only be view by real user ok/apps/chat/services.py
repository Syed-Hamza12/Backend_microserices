...
#: Safety net for the internal <untrusted_data> boundary marker (see
#: prompt.wrap_untrusted) leaking into model output verbatim — the model is
#: instructed never to echo it, but an 8B model under JSON-mode pressure has
#: reproduced it anyway, which both looks broken to the owner and, worse, can
#: push the raw JSON generation past what the JSON-mode validator accepts
#: (an unescaped/truncated string is how "json_validate_failed" happens).
#: Stripped from every string field before validation so a leak never reaches
#: the owner even if the prompt-level instruction is ignored.
_UNTRUSTED_TAG_RE = re.compile(r"</?untrusted_data>")


def _strip_untrusted_markers(value):
    if isinstance(value, str):
        return _UNTRUSTED_TAG_RE.sub("", value)
    if isinstance(value, dict):
        return {k: _strip_untrusted_markers(v) for k, v in value.items()}
    if isinstance(value, list):
        return [_strip_untrusted_markers(v) for v in value]
    return value


def _parse_and_validate(raw_content):
    data = json.loads(raw_content)
    data = _strip_untrusted_markers(data)
...
def generate_reply(*, business, conversation, text, language=None):
    ...
    messages = prompt.build_messages(
        business, history, text, language=language, conversation=conversation, tier=tier
    )
