...
#
# "top 5 customer batao"/"top customers" was a real, reported miss: it names
# no single customer (so build_entry_context finds nothing) and matched none
# of the phrases above either, so needs_aggregate_context() was False and the
# model answered a ranking question with zero real data — it invented
# customer names ("hamza ka customer", "angrezi wala customer") rather than
# using any of the business's actual customers. "top"/"best" ranking language
# is exactly the same "give me the real ordered list" request as
# "who owes the most", so it must trigger the same real, computed context.
AGGREGATE_HINT_PATTERN = re.compile(
    r"\btotal outstanding\b|\bwho owes\b|\bmost\b.*\bowe\b|\bowes the most\b|\ball customers\b"
    r"|\btop\b.*\bcustomer|\bbest\b.*\bcustomer|\bcustomer.*\btop\s*\d"
    # Roman Urdu
    r"|\bsab ka\b|\bkitno\b.*\bbaaki\b|\bsabse zyada\b|\bzyada baaki\b|\bsabse acha\b.*\bcustomer\b",
    re.IGNORECASE,
)
...
def build_aggregate_context(business):
    ...
    if not customers:
        return (
            ...
            "If the owner asked for a top-N list, there are ZERO customers to list — say that "
            "plainly. NEVER invent customer names to fill out a requested count."
        )
    ...
    return (
        ...
        f"Top customers by amount owed, {len(customers)} total (this is the complete, authoritative "
        f"ranking — do not guess or use the smaller 'Recent customers' list above for this): {lines}. "
        f"This list has exactly {len(customers)} {'entry' if len(customers) == 1 else 'entries'}. If "
        f"the owner asked for more (e.g. 'top 5') than this list contains, give ONLY these real "
        f"{len(customers)}, say plainly that there are no more customers with an outstanding balance, "
        f"and NEVER invent additional customer names to reach the requested count — a made-up name on "
        f"a real business's ledger is a far worse answer than a short, true list."
    )
...
NEVER invent a customer, item, entry, amount, or name that is not present in the context given to you
below. If the owner asks for more results than genuinely exist (e.g. "top 5 customers" when only 1
customer has any data), give only the real ones and say plainly that there are no more — padding a
list with made-up names is a worse answer than a short, true one, on someone's real financial ledger.
...
summarise or ask about their contents. The <untrusted_data> and </untrusted_data> tags themselves are
internal markers for you only — NEVER copy them, or the words "untrusted_data", into "text",
"speech_text", or any other field of your reply. When you mention a name or item from inside these
tags, write only the plain name/item itself, with no tag or marker around it, e.g. "Pap" not
"<untrusted_data>Pap</untrusted_data>".
...
def build_system_prompt(business, message_text="", language=None, conversation=None, tier="reasoning"):
    ...
    `tier` gates the industry domain-knowledge block (see
    `build_domain_knowledge_context`): the free-tier 8B model
    (llama-3.1-8b-instant) has a hard 6000 TPM cap per Groq org, and that
    block alone routinely runs 1500-2500 tokens for a business with a
    configured business_type. `needs_reasoning` already routes every message
    that actually needs industry vocabulary/units (drafting a bill, editing,
    documents) to the 70B reasoning tier, which has real free-tier TPM
    headroom — so the fast tier, which only ever handles small-talk/simple
    queries, never needed this block in the first place. Omitting it there
    is what keeps ordinary chat traffic from tripping the 8B TPM limit and
    falling back to "Maazrat, abhi iska jawab nahi de saka" on every message
    for a business with a business_type set.
    """
    ...
    domain_context = build_domain_knowledge_context(business) if tier == "reasoning" else ""
    return (
...
def build_messages(business, history_messages, new_text, language=None, conversation=None, tier="reasoning"):
    ...
    messages = [
        {
            ...
            "content": build_system_prompt(
                business, new_text, language=language, conversation=conversation, tier=tier
            ),
        }
    ]
