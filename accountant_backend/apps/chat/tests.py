...
class SaveNowSendIntentTests(TestCase):
    ...
    def test_send_wording_triggers_a_delivery_attempt_and_says_it_failed(self):
        ...
        with mock.patch("apps.chat.services.call_groq", return_value=json.dumps(payload)):
            reply = services.generate_reply(
                business=self.business, conversation=self.conversation,
...
class ModelTierSelectionTests(TestCase):
    ...
    see test_generate_reply_never_touches_ocr_gemini_client. See
    ModelFallbackTests for what happens when Groq's Roman
...
class ModelFallbackTests(TestCase):
    ...
    different model. Gemini is reserved for OCR + chat's fast tier only
    (see ModelTierSelectionTests)."""
